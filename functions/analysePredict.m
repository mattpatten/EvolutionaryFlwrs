function analysePredict(expID, disp_fig)
...
[numTrials, ~, numPiecases, numSubjects] = size(allPieVals{1});
...
for p=1:numParams
    ...
    num_conv_lower{p} = sum(all_conv_lower{p}<(numTrials+1-epsilon));
    num_conv_upper{p} = sum(all_conv_upper{p}<(numTrials+1-epsilon));
end

excel_lower = display_rank(mean_conv_lower, percentile_lower, num_conv_lower, numTrials, numSubjects, epsilon, roulette_vals, roulette_str, 'lower');
excel_upper = display_rank(mean_conv_upper, percentile_lower, num_conv_upper, numTrials, numSubjects, epsilon, roulette_vals, roulette_str, 'upper');

excel = [excel_lower; {' ',' ',' ',' ',' '}; excel_upper];
...
function [excel] = display_rank(mean_conv, percentile_lower, num_conv, numTrials, numSubjects, epsilon, roulette_vals, roulette_str, analysis_type)
...
rank = 1;
excel = {'Rank','Parameter','Value','Time to convergence',['Number of subjects converged (out of ' num2str(numTrials) ')']};
for i=1:length(sorted_conv)
    ...
    for idx=1:length(pp) %if multiple parameters have this specific time to convergence (e.g., if a parameter has only two values - the time to convergence will be identical, or if the value hasn't converged for anybody)
        param = pp(idx);
        val = vv(idx);
        if percentile_lower{param}(val)>0
