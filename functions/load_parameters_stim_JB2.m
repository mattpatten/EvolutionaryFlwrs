function [st] = load_parameters_stim_MP
...
%% Region sizes
st.h.size                   = 0.06;       %[0-Inf) radius of each hole
st.h.var                    = 0;       %[0-Inf) random variance (%) added in hole size
...
st.d.size                   = [0.60; 0.65;]       %radius of central disk
st.d.size_x_offset          = 0.05;       %(-Inf to Inf) the change in the radius along the x-axis, if we want the disk to be non-circular (can use both -/+)
...
st.t.sizeX                  = 0.45;       %[0-Inf) the length (radius) of the ellipse for trans florets
st.t.sizeY                  = 0.25;       %[0-Inf) the height (radius) of the ellipse for trans florets
st.t.varX                   = [0; 0.07; .15; .3]; %[0-Inf) random variance (%) added in individual trans lengths
st.t.varY                   = [0; 0.07; .15; .3];       %[0-Inf) random variance (%) added in individual trans widths

st.r.sizeX                  = 0.87;       %[0-Inf) the length (radius) of the ellipse for ray florets
st.r.sizeY                  = 0.28; %[0-Inf) the height (radius) of the ellipse for ray florets
st.r.varX                   = [0; 0.07; .15; .3];       %[0-Inf) random variance (%) added in individual petal lengths
st.r.varY                   = [0; 0.07; .15; .3];       %[0-Inf) random variance (%) added in individual trans widths
...
%% Angular tips
st.t.tip_length             = 0.4;        %(0-Inf) the length (in visual degrees) of the tip, extending out radially. This is measured from the connection point outwards (not the end of the ellipse)
st.t.tips_sharpness         = 2; %(0-Inf) the sharpness of the tips/edges of the ray florets, of the order x.^n (i.e., 1 is linear/45 deg point, 2 is parabolic, 100 is absolutely flat)
st.t.tips_likelihood_change = [0; 0.2; 0.5];          %[0-1] the likelihood that each petal's tip will change from the original setting.
st.t.tips_other_sharpnesses = [1 10]; %(0-Inf) if the sharpness changes for a petal, the other sharpnesses it may be instead
                            
st.r.tip_length             = 0.6;        %(0-Inf) the length (in visual degrees) of the tip, extending out radially. This is measured from the connection point outwards (not the end of the ellipse)
st.r.tips_sharpness         = 2;  %(0-Inf) the sharpness of the tips/edges of the ray florets, of the order x.^n (i.e., 1 is linear/45 deg point, 2 is parabolic, 100 is absolutely flat)
st.r.tips_likelihood_change = [0; 0.2; 0.5];          %[0-1] the likelihood that each petal's tip will change from the original setting.
st.r.tips_other_sharpnesses = [1 10]; %(0-Inf) if the sharpness changes for a petal, the other sharpnesses it may be instead
...
st.t.orientation_shift_var  = [0; 20; 90];           %[0-180] the amount of variance (in degrees, ori_shift ± var) in placing the position/orientation of the next trans floret
st.r.orientation_shift      = 137.5;      %[0-360) the change in orientation with the generation of every new ray floret
st.r.orientation_shift_var  = [0; 20; 90]; %[0-180] the amount of variance (in degrees, ori_shift ± var) in placing the position/orientation of the next ray floret
...
st.t.avgPetals              = [6; 18; 33];         %[1-inf) number of trans florets to generate
st.t.avgPetals_var          = 3;          %[0-inf) the amount of variance (nPetals ± var) in the number of trans florets that might be displayed.
st.r.avgPetals              = [7; 24; 44]; %[1-inf) number of ray florets to generate 
st.r.avgPetals_var          = 4;          %[0-inf) the amount of variance (nPetals ± var) in the number of ray florets that might be displayed.


%% Holes - Unique Properties
st.h.nRows                  = 3;          %[1-inf)  how many rows of holes (distances from the centre) we want in the disk florets
st.h.avgHoles               = [5; 13; 20];   %[1-inf)  number of holes to put inside disk florets, distributed proportionally across the rows (closer to the centre has less compared to close to the outside)
...
st.h.row_std                = 0.2;        %[0-1]    variation in the distance from the centre within each row; normal curve peaking at each row distance.   0=all the same distance; 1=varies between rows on either side of it
st.h.ori_std                = 0.2;        %[0-1]    variation in the orientation of each hole within each row; normal curve peaking at its intended spread. 0=all equally apart around the circle for this row; 1=varies between neighbouring spots on same row
...
st.h.draw_outline           = 1;          %[0,1] draw black outline around edge of shape (1) or not (0)
st.d.draw_outline           = 1; 
st.t.draw_outline           = 1; 
st.r.draw_outline           = 1; 
...
st.h.fractal_exponent       = [0; 1; 2];        %[0-2, but also up to ~5], exponent / slope value of fractals.
st.d.fractal_exponent       = [0; 1; 2];
st.t.fractal_exponent       = [0; 1; 2];
st.r.fractal_exponent       = [0; 1; 2];

st.h.fractal_type           = [0; 1];
st.d.fractal_type           = [0; 1];
st.t.fractal_type           = [0; 1];
st.r.fractal_type           = [0; 1];
...
st.h.colour                 = [.6 .6 .6]; %RGB triplets [0-1]. e.g., [0 1 1] (Red, green, blue)
st.d.colour                 = [.6 .6 .6]; %Some predefined RGB triplets can be seen by typing "rgb chart" and used via rgb('name')
st.t.colour                 = [.6 .6 .6];
st.r.colour                 = [.6 .6 .6]; 
