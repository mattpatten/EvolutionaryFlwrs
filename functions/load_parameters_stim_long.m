...
st.d.size                   = [0.60; 0.65];         %diameter of central disk
...
st.t.sizeY                  = [0.20; 0.24; 0.28];   %[0-Inf) the height (radius) of the ellipse for trans florets
st.t.varX                   = [0; 0.07; .15; .25];   %[0-Inf) random variance (%) added in individual trans lengths
...
st.r.sizeY                  = [0.22; 0.27; 0.32];   %[0-Inf) the height (radius) of the ellipse for ray florets
st.r.varX                   = [0; .07; .15; .25];   %[0-Inf) random variance (%) added in individual petal lengths
...
st.t.tips_sharpness         = [0.80; 2.5; 3.0]; %(0-Inf) the sharpness of the tips/edges of the ray florets, of the order x.^n (i.e., 1 is linear/45 deg point, 2 is parabolic, 100 is absolutely flat)
...
st.r.tips_sharpness         = [0.8; 2; 3.0];    %(0-Inf) the sharpness of the tips/edges of the ray florets, of the order x.^n (i.e., 1 is linear/45 deg point, 2 is parabolic, 100 is absolutely flat)
...
st.t.orientation_shift_var  = [0; 20; 180];     %[0-180] the amount of variance (in degrees, ori_shift ± var) in placing the position/orientation of the next trans floret
st.r.orientation_shift      = 137.5;            %[0-360) the change in orientation with the generation of every new ray floret
st.r.orientation_shift_var  = [0; 30; 180];     %[0-180] the amount of variance (in degrees, ori_shift ± var) in placing the position/orientation of the next ray floret
...
st.t.avgPetals              = [5; 14; 23; 33];  %[6; 18; 33];      %[1-inf) number of trans florets to generate
st.t.avgPetals_var          = 3;                %[0-inf) the amount of variance (nPetals ± var) in the number of trans florets that might be displayed.
st.r.avgPetals              = [5; 14; 27; 44];  %[1-inf) number of ray florets to generate 
...
st.h.fractal_type           = 0; 
st.d.fractal_type           = [0; 1; 2];
st.t.fractal_type           = [0; 1; 2];
st.r.fractal_type           = [0; 1; 2];  %(where X is flower region 'h','d','t',or'r')fractal_type summarises the top two variables. 0 means no fractal is shown, 1 means the regular fractal is displayed, and 2 means the thresholded/binary/ugly fractal is presented. It only takes 3 values. 
...
st.t.colour                 = [0.78431 0.22745 0.18431; 0.8902 0.54902 0.63922; 0.35686 0.69412 0.39216; 0.97255 0.83137 0.24314];
st.r.colour                 = [0.78431 0.22745 0.18431; 0.8902 0.54902 0.63922; 0.35686 0.69412 0.39216; 0.97255 0.83137 0.24314]; 
