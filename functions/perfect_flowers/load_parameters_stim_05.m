...
st.t.varX                   = [0.07; .07; .07];   %[0-Inf) random variance (%) added in individual trans lengths
...
st.r.varX                   = [0; 0];   %[0-Inf) random variance (%) added in individual petal lengths
...
st.t.tips_sharpness         = [0.80; 0.80; 0.80]; %(0-Inf) the sharpness of the tips/edges of the ray florets, of the order x.^n (i.e., 1 is linear/45 deg point, 2 is parabolic, 100 is absolutely flat)
...
st.r.tips_likelihood_change = [0.2; 0.2];    %[0-1] the likelihood that each petal's tip will change from the original setting.
