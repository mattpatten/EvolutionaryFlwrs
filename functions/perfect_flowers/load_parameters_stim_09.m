function [st] = load_parameters_stim_long
...
st.t.sizeY                  = [0.2; 0.2; 0.2];   %[0-Inf) the height (radius) of the ellipse for trans florets
...
st.r.sizeY                  = [0.32; 0.32; 0.32];   %[0-Inf) the height (radius) of the ellipse for ray florets
st.r.varX                   = [0.15; .15; .15; .15];   %[0-Inf) random variance (%) added in individual petal lengths
...
st.t.tips_sharpness         = [0.80; 0.8; 0.8]; %(0-Inf) the sharpness of the tips/edges of the ray florets, of the order x.^n (i.e., 1 is linear/45 deg point, 2 is parabolic, 100 is absolutely flat)
st.t.tips_likelihood_change = 0;                %[0-1] the likelihood that each petal's tip will change from the original setting.
st.t.tips_other_sharpnesses = [1.4 2.5];         %(0-Inf) if the sharpness changes for a petal, the other sharpnesses it may be instead
...
st.r.tips_sharpness         = [2; 2; 2];    %(0-Inf) the sharpness of the tips/edges of the ray florets, of the order x.^n (i.e., 1 is linear/45 deg point, 2 is parabolic, 100 is absolutely flat)
st.r.tips_likelihood_change = [0.5; 0.5; 0.5];    %[0-1] the likelihood that each petal's tip will change from the original setting.
st.r.tips_other_sharpnesses = [1.4 2.5];        %(0-Inf) if the sharpness changes for a petal, the other sharpnesses it may be instead
...
st.t.avgPetals              = [14; 14; 33; 33];  %tied both of these equally %[1-inf) number of trans florets to generate
st.t.avgPetals_var          = 3;                %[0-inf) the amount of variance (nPetals ± var) in the number of trans florets that might be displayed.
st.r.avgPetals              = [27; 27; 27; 27];  %[1-inf) number of ray florets to generate 
...
st.h.avgHoles_var           = 0;                %[0-inf)  the amount of variance (nHoles ± var) in the number of holes that might be displayed.
st.h.avgHoles               = 0;                %(-inf,inf)  number of holes to put inside disk florets, distributed proportionally across the rows (closer to the centre has less compared to close to the outside)
...
st.h.fractal_type           = 0; 
st.d.fractal_type           = [0; 0; 0];
st.t.fractal_type           = [0; 0; 0];
st.r.fractal_type           = [0; 0; 0];  %(where X is flower region 'h','d','t',or'r')fractal_type summarises the top two variables. 0 means no fractal is shown, 1 means the regular fractal is displayed, and 2 means the thresholded/binary/ugly fractal is presented. It only takes 3 values. 
...
st.t.colour                 = [0.97255 0.83137 0.24314; 0.97255 0.83137 0.24314; 0.97255 0.83137 0.24314; 0.97255 0.83137 0.24314];
st.r.colour                 = [0.97255 0.83137 0.24314; 0.97255 0.83137 0.24314; 0.97255 0.83137 0.24314; 0.97255 0.83137 0.24314]; 
