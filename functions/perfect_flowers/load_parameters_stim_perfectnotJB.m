...
st.t.varX                   = [.07; .07; .07; .07]; %[0-Inf) random variance (%) added in individual trans lengths
st.t.varY                   = [0.3; 0.3; 0.3; 0.3];       %[0-Inf) random variance (%) added in individual trans widths
...
st.r.varX                   = [0.3; 0.3; .3; .3];       %[0-Inf) random variance (%) added in individual petal lengths
st.r.varY                   = [0.3; 0.3; .3; .3];       %[0-Inf) random variance (%) added in individual trans widths
...
st.t.tips_likelihood_change = [0.5; 0.5; 0.5];          %[0-1] the likelihood that each petal's tip will change from the original setting.
...
st.r.tips_likelihood_change = [0.5; 0.5; 0.5];          %[0-1] the likelihood that each petal's tip will change from the original setting.
...
st.t.orientation_shift_var  = [20; 20; 20];           %[0-180] the amount of variance (in degrees, ori_shift ± var) in placing the position/orientation of the next trans floret
st.r.orientation_shift      = 137.5;      %[0-360) the change in orientation with the generation of every new ray floret
st.r.orientation_shift_var  = [90; 90; 90]; %[0-180] the amount of variance (in degrees, ori_shift ± var) in placing the position/orientation of the next ray floret
...
st.t.avgPetals              = [18; 18; 18];         %[1-inf) number of trans florets to generate
st.t.avgPetals_var          = 3;          %[0-inf) the amount of variance (nPetals ± var) in the number of trans florets that might be displayed.
st.r.avgPetals              = [7; 7; 7]; %[1-inf) number of ray florets to generate 
...
st.h.avgHoles               = [5; 5; 5];   %[1-inf)  number of holes to put inside disk florets, distributed proportionally across the rows (closer to the centre has less compared to close to the outside)
...
st.h.fractal_exponent       = [2; 2; 2];        %[0-2, but also up to ~5], exponent / slope value of fractals.
st.d.fractal_exponent       = [1; 1; 1];
st.t.fractal_exponent       = [1; 1; 1];
st.r.fractal_exponent       = [1; 1; 1];

st.h.fractal_type           = [1; 1];
st.d.fractal_type           = [0; 0];
st.t.fractal_type           = [1; 1];
st.r.fractal_type           = [0; 0];
